...
class Solution {
    public int maxProfit(int k, int[] prices) {
        ...
        for(int i=0;i<n;i++) for(int j=0;j<2;j++) for(int z=0;z<=k;z++) dp[i][j][z] = -1;
        return maxProfit(0, prices, 1, k, n, dp);
    }
    public int maxProfit(int day, int[] prices, int state, int transactions, int n, int[][][] dp){
        if(day == n || transactions == 0) return 0;
...
class Solution {
    public int maxProfit(int k, int[] prices) {
        ...
        // base case
        for(int j=0;j<2;j++) for(int z=0;z<=k;z++) dp[n][j][z] = 0;
...
class Solution {
    public int maxProfit(int k, int[] prices) {
        ...
        // base case
        for(int j=0;j<2;j++) for(int z=0;z<=k;z++) dp[j][z] = 0;
